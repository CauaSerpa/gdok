...
<div class="app-sidebar-menu">
    <div class="h-100" data-simplebar>
        ...
        <div id="sidebar-menu">
            ...
            <div class="logo-box">
                <a href="<?= INCLUDE_PATH_DASHBOARD . "portal/{$office['link']}/"; ?>" class="logo logo-light">
                    <span class="logo-sm">
                ...
                <a href="<?= INCLUDE_PATH_DASHBOARD . "portal/{$office['link']}/"; ?>" class="logo logo-dark">
                    <span class="logo-sm">
                        <img src="<?= INCLUDE_PATH_DASHBOARD; ?>assets/images/logo-sm.png" alt="" height="22">
                    </span>
                    <span class="logo-lg">
                ...
                <?php if (isset($tab) && !empty($tab)): ?>
                    ...
                    <?php if ($departments): ?>
                        <?php foreach ($departments as $department): ?>
                            <?php
                                // Consulta para obter categorias únicas que possuem documentos para o departamento
                                $stmt = $conn->prepare("
                                    SELECT DISTINCT c.*
                                ...
                                ");
                                $stmt->execute([$department['id'], $_SESSION['user_id'], $token]);
                                    ...
                                    <div class="collapse" id="sidebar<?= $department['id']; ?>">
                                        <ul class="nav-second-level">
                                            <?php foreach ($categories as $category): ?>
                                                <li>
                                                    <a href="<?= $urlCompleta; ?>?depto=<?= $department['id']; ?>&categoria=<?= $category['id']; ?>" class="tp-link">
                                                        <?= $category['name']; ?>
                                                    </a>
                                                </li>
                                            <?php endforeach; ?>
