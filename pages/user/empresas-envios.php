<style>
    #companiesSendingsTable_paginate .pagination {
        display: none;
    }
    .dataTables_empty {
        text-align: center;
...
<div class="py-3 d-flex align-items-sm-center flex-sm-row flex-column">
    <div class="flex-grow-1">
        <h4 class="fs-18 fw-semibold m-0">Empresas de Envios</h4>
    </div>

    <div class="text-end">
        <a href="<?= INCLUDE_PATH_DASHBOARD; ?>cadastrar-empresa-envio" class="btn btn-dark">
            <i class="mdi mdi-plus fs-16 align-middle"></i>
            Cadastrar Empresa de Envio
        </a>
    </div>
</div>
        ...
        <div class="card overflow-hidden mb-0">
            <div class="card-header">
                <div class="d-flex align-items-center">
                    <h5 class="card-title text-black mb-0">Empresas de Envio cadastradas</h5>
                    <div class="ms-auto">
                        <button class="btn btn-sm bg-light border dropdown-toggle fw-medium text-black" type="button" data-bs-toggle="offcanvas" href="#filterOffcanvas" role="button" aria-controls="filterOffcanvas">
                            <i class="mdi mdi-filter-outline me-1 fs-14"></i>Filtrar Empresas de Envios</i>
                        </button>
                    </div>
                </div>
            ...
            <div class="card-body p-0">
                <table id="companiesSendingsTable" class="table table-traffic mb-0">
                    <thead>
                        <tr>
                            <th>Nome da Empresa de Envio</th>
                            <th>Telefone</th>
                            <th>E-mail</th>
                            <th>Responsável</th>
                            <th>Ações</th>
...
<script>
    $(document).ready(function () {
        ...
        $(document).on('click', '.btn-delete', function () {
            ...
            $('#companySendingName').text(elementNameToDelete); // Define o nome no modal
...
<script>
    $(document).ready(function () {
        ...
        const table = $('#companiesSendingsTable').DataTable({
            processing: true,
            serverSide: true,
            searching: false,
            ordering: true,
            paging: true,
            ajax: {
                url: '<?= INCLUDE_PATH_DASHBOARD; ?>back-end/user/company-sending/list.php',
                type: 'GET',
                data: function (d) {
