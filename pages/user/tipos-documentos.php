<style>
    #documentTypes_paginate .pagination {
        display: none;
    }
    .dataTables_empty {
        text-align: center;
...
<div class="py-3 d-flex align-items-sm-center flex-sm-row flex-column">
    <div class="flex-grow-1">
        <h4 class="fs-18 fw-semibold m-0">Tipos de Documentos</h4>
    </div>

    <div class="text-end">
        <a href="<?= INCLUDE_PATH_DASHBOARD; ?>cadastrar-tipo-documento" class="btn btn-dark">
        ...
        <div class="card overflow-hidden mb-0">
            <div class="card-header">
                <div class="d-flex align-items-center">
                    <h5 class="card-title text-black mb-0">Tipos de documentos cadastradas</h5>
                </div>
            </div>

            <div class="card-body p-0">
                <table id="documentTypes" class="table table-traffic mb-0">
                    <thead>
                        <tr>
                            <th>Nome</th>
                            <th>Categoria</th>
...
<script>
    $(document).ready(function () {
        ...
        $(document).on('click', '.btn-delete', function () {
            ...
            $('#documentTypeName').text(elementNameToDelete); // Define o nome no modal
            ...
            if (elementIdToDelete) {
                ...
                $.ajax({
                    ...
                    error: function (xhr, status, error) {
                        ...
                        $("#documentsTypesForm").before('<div class="alert alert-danger alert-dismissible fade show w-100" role="alert">Ocorreu um erro, tente novamente mais tarde.<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>');
                    }
                });
            }
        });
...
<script>
    $(document).ready(function () {
        ...
        const table = $('#documentTypes').DataTable({
            processing: true,
            serverSide: true,
            searching: false,
            ordering: true,
            paging: true,
            ajax: {
                url: '<?= INCLUDE_PATH_DASHBOARD; ?>back-end/user/document-types/list-document-types.php', // URL para carregar os dados
                type: 'GET',
