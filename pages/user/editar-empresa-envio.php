    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                Tem certeza de que deseja excluir a empresa de envio "<span id="companySendingName"></span>"?
            </div>
...
<?php
    ...
    if (!isset($token) || empty($token)) {
        ...
        $message = array('status' => 'error', 'alert' => 'danger', 'title' => 'Error', 'message' => 'Token inválido ou ausente. Não foi possível carregar a empresa de envio.');
        $_SESSION['msg'] = $message;
        header('Location: ' . INCLUDE_PATH_DASHBOARD . 'empresas-envios');
        exit;
    }

    include('back-end/user/company-sending/functions.php');

    // Validar o token (por exemplo, verificar se existe no banco de dados)
    $company_sending_id = $token;
    $companySending = getEmpresaByToken($company_sending_id, $conn); // Suponha que essa função pegue a empresa pelo token

    // Se não encontrar a empresa com o token, redirecionar
    if (!$companySending) {
        // Defina a mensagem de erro na sessão
        $message = array('status' => 'error', 'alert' => 'danger', 'title' => 'Error', 'message' => 'Empresa de envio não encontrada.');
        $_SESSION['msg'] = $message;
        header('Location: ' . INCLUDE_PATH_DASHBOARD . 'empresas-envios');
        exit;
    }
...
<div class="container">
    <div class="py-3 d-flex align-items-sm-center flex-sm-row flex-column">
        <div class="flex-grow-1">
            <h4 class="fs-18 fw-semibold m-0">Editar Empresa de Envio</h4>
        </div>
    
        <div class="text-end">
            <ol class="breadcrumb m-0 py-0">
                <li class="breadcrumb-item"><a href="<?= INCLUDE_PATH_DASHBOARD; ?>empresas-envios">Empresas Envios</a></li>
                <li class="breadcrumb-item active">Editar Empresa de Envio</li>
            </ol>
        </div>
    </div>
        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-header">
                    <h5 class="card-title mb-0">Edição de Empresas de Envio</h5>
                </div><!-- end card header -->

                <div class="card-body">

                    <form id="companySendingForm">

                        <!-- Name -->
                        <div class="mb-3">
                            <label for="name" class="form-label">Nome da Empresa</label>
                            <input class="form-control" name="name" type="text" id="name" maxlength="120" placeholder="Digite o Nome da Empresa" value="<?= $companySending['name']; ?>" required>
                        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input class="form-control" name="responsible" type="text" id="responsible" maxlength="100" placeholder="Digite o Nome do Responsável" value="<?= $companySending['responsible']; ?>" required>
                                </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-5">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input class="form-control" name="phone" type="tel" id="phone" maxlength="15" placeholder="Digite o Telefone" value="<?= $companySending['phone']; ?>" onkeyup="handlePhone(event)" required>

                                    <div class="form-check form-switch">
                                        <input class="form-check-input" name="notify_phone" type="checkbox" role="switch" id="notify_phone" value="1" <?php if ($companySending['notify_phone']) echo 'checked'; ?>>
                            ...
                            <div class="col-md-7">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input class="form-control" name="email" type="email" id="email" maxlength="120" placeholder="Digite o E-mail" value="<?= $companySending['email']; ?>" required>

                                    <div class="form-check form-switch">
                                        <input class="form-check-input" name="notify_email" type="checkbox" role="switch" id="notify_email" value="1" <?php if ($companySending['notify_email']) echo 'checked'; ?>>
                        ...
                        <div class="d-flex align-items-center justify-content-between">
                            ...
                            <div>
                                ...
                                <button class="btn btn-danger btn-delete" type="button" data-id="<?= $companySending['id']; ?>" data-name="<?= $companySending['name']; ?>">Excluir</button>
...
<script>
    $(document).ready(function () {
        ...
        $('.btn-delete').on('click', function () {
            ...
            $('#companySendingName').text(elementNameToDelete); // Mostra o modal
        ...
        $('#confirmDelete').on('click', function () {
            if (elementIdToDelete) {
                ...
                $.ajax({
                    url: `<?= INCLUDE_PATH_DASHBOARD; ?>back-end/user/company-sending/delete.php?id=${elementIdToDelete}`,
                    type: 'DELETE',
                    ...
                    error: function (xhr, status, error) {
                        ...
                        $("#companySendingForm").before('<div class="alert alert-danger alert-dismissible fade show w-100" role="alert">Ocorreu um erro, tente novamente mais tarde.<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>');
                    }
                });
            }
        });
...
$(document).ready(function() {
    ...
    $.validator.addMethod("documentExists", function(value, element) {
        ...
        if (value) {
            $.ajax({
                url: "<?= INCLUDE_PATH_DASHBOARD; ?>back-end/user/company-sending/forms-validations/document-exists.php",
                type: "POST",
                data: { action: 'document-exists', document: value, company_id: <?= $company_sending_id; ?> },
                async: false, // Sincronizar para garantir a validação antes de prosseguir
                success: function(response) {
                    isValid = response.status === "available"; // Verifica se o e-mail está disponível
                },
    ...
    $("#companySendingForm").validate({
        ...
        submitHandler: function(form) {
            ...
            formData.append("company_sending_id", <?= $company_sending_id; ?>);
            formData.append("action", "update-company-sending");

            // Realiza o AJAX para enviar os dados
            $.ajax({
                url: '<?= INCLUDE_PATH_DASHBOARD; ?>back-end/user/company-sending/update.php', // Substitua pelo URL do seu endpoint
                type: 'POST',
                data: formData,
                ...
                success: function (response) {
                    if (response.status == "success") {
                        ...
                        $(".alert").remove(); // Remove qualquer mensagem de erro anterior
                        $("#companySendingForm").before('<div class="alert alert-danger alert-dismissible fade show w-100" role="alert">' + response.message + '<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>');
                    }
