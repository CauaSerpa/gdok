...
<div class="py-3 d-flex align-items-sm-center flex-sm-row flex-column">
    <div class="flex-grow-1">
        <div class="clearfix">
            ...
            <div class="float-end">
                <div class="d-grid">
                    <label for="company" class="form-label">Empresas</label>
                    <select id="company" class="form-select">
                        <option value="">Selecione uma empresa</option>
                        <?php foreach ($companiesSelect as $companySelect): ?>
                            <option value="<?= htmlspecialchars($companySelect['id']); ?>"<?= (isset($company) && $company == $companySelect['id']) ? "selected" : ""; ?>>
                                <?= htmlspecialchars($companySelect['name']); ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
...
<?php
    if ($department) {
        ...
        $stmt = $conn->prepare("
            SELECT sd.*
            FROM tb_sending_departments sd
            WHERE sd.id = ?
        ");
...
<script>
    ...
    $(document).ready(function() {
        $('#company').change(function() {
            let selectedCompanyId = $(this).val(); // Obtém o ID da empresa selecionada
            let currentUrl = window.location.href; // Pega a URL atual

            // Remove os parâmetros GET
            let urlWithoutQuery = currentUrl.split('?')[0];

            // Remove qualquer ID de empresa existente (assumindo que esteja na última parte da URL)
            let baseUrl = urlWithoutQuery.replace(/\/\d+$/, '');

            if (selectedCompanyId) {
                window.location.href = baseUrl + '/' + selectedCompanyId; // Adiciona ou substitui o ID da empresa na URL
            } else {
                window.location.href = baseUrl; // Se nenhuma empresa for selecionada, mantém a URL base
            }
        });
    });
</script>
    ...
    $(document).ready(function () {
        const table = $('#documentsTable').DataTable({
            ...
            columns: [
                ...
                { data: 'price', className: 'text-end', width: '7.5%' },
                { data: 'expiration_date', className: 'text-end', width: '15%' },
                { data: 'upload_date', className: 'text-end', width: '15%' },
                { data: 'document', className: 'text-center', width: '7.5%' },
                {
                    data: 'status',
                    className: 'text-nowrap text-center',
                    width: '7.5%'
                },
                { data: 'read_in', className: 'text-end', width: '15%' },
            ],
            buttons: [
                {
                    extend: 'csvHtml5',
