...
if ($_SERVER["REQUEST_METHOD"] === "GET") {
    try {
        ...
        $queryTotal = "SELECT COUNT(*) as total 
                       FROM tb_document_type_categories 
        ...
        $query = "SELECT id, name FROM tb_document_type_categories WHERE user_id = :user_id";
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $actions = '<a href="' . INCLUDE_PATH_DASHBOARD . 'editar-categoria/' . $row['id'] . '" class="btn btn-sm bg-primary-subtle me-1" data-bs-toggle="tooltip" title="Edit">
