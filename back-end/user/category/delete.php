    ...
    if ($_SERVER['REQUEST_METHOD'] === 'DELETE' && isset($_GET['id'])) {
        ...
        try {
            ...
            if ($result['count'] > 0) {
                ...
                $message = array('status' => 'error', 'alert' => 'danger', 'title' => 'Error', 'message' => 'Esta categoria está em uso. Não pode ser excluída.');
                $_SESSION['msg'] = $message;
                exit;
            }

            // Executa a exclusão da categoria
            $stmt = $conn->prepare("DELETE FROM tb_document_type_categories WHERE id = ?");
            $stmt->execute([$id]);
            ...
            if ($stmt->rowCount()) {
                ...
                $message = array('status' => 'success', 'alert' => 'primary', 'title' => 'Sucesso', 'message' => 'Categoria deletada com sucesso.');
                $_SESSION['msg'] = $message;
            } else {
            ...
            // Log do erro
            error_log("Erro ao deletar a categoria: " . $e->getMessage());
