<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['action']) && $_POST['action'] === "resend-code") {
        if (isset($_SESSION['user_id'])) {
            ...
            try {
                ...
                // Reenviar o código de verificação para o e-mail do usuário (ou via SMS, etc.)
                ...
                echo json_encode(['status' => 'success', 'message' => 'Código de verificação reenviado para ' . $user['email'] . '. Verifique seu e-mail.']);
            } catch (Exception $e) {
                // Registrar erro em um log
                error_log("Erro ao reenviar e-mail de verificação: " . $e->getMessage());

                // Mensagem genérica para o usuário
                echo json_encode(['status' => 'error', 'message' => 'Erro ao reenviar e-mail de verificação.', 'error' => $e->getMessage()]);
            }
        } else {
