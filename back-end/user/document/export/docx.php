<?php
    ...
    function generateDocx($data) {
        ...
        // Adiciona os dados ao DOCX
        $section->addTitle('Documentos Exportados', 1);
        $table = $section->addTable();

        // Definindo o cabeçalho da tabela
        $table->addRow();
        $table->addCell(2000)->addText('Company');
        $table->addCell(2000)->addText('Document Type');
        $table->addCell(2000)->addText('Document');
        $table->addCell(2000)->addText('Expiration Date');
        $table->addCell(2000)->addText('Advance Notification');
        $table->addCell(2000)->addText('Status');

        // Preenche a tabela com os dados
        foreach ($data as $row) {
            $table->addRow();
            $table->addCell(2000)->addText($row['company']);
            $table->addCell(2000)->addText($row['document_type']);
            $table->addCell(2000)->addText($row['document']);
            $table->addCell(2000)->addText($row['expiration_date']);
            $table->addCell(2000)->addText($row['advance_notification']);
            $table->addCell(2000)->addText($row['status']);
        }

        // Salva o arquivo DOCX diretamente na memória
        $objWriter = \PhpOffice\PhpWord\IOFactory::createWriter($phpWord, 'Word2007');
        
        // Cria um buffer para o arquivo DOCX
        ob_start();
        $objWriter->save("php://output"); // Envia para a saída sem salvar no servidor
    ...
    if ($_SERVER["REQUEST_METHOD"] === "GET") {
        try {
            ...
            $fileContent = generateDocx($data);
    
            // Envia o arquivo gerado para o download
            header('Content-Type: application/vnd.openxmlformats-officedocument.wordprocessingml.document');
            header('Content-Disposition: attachment; filename="documentos_' . date('Y-m-d_H-i-s') . '.docx"');
            echo $fileContent; // Exibe diretamente o conteúdo gerado
            exit;
