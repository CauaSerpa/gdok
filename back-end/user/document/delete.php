<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'DELETE' && isset($_GET['id'])) {
        ...
        try {
            ...
            $stmt = $conn->prepare("SELECT id FROM tb_documents WHERE id = ?");
            $stmt->execute([$id]);
            $document = $stmt->fetch();

            if ($document) {
                // Deleta a entrada no banco de dados
                $stmt = $conn->prepare("DELETE FROM tb_documents WHERE id = ?");
                $stmt->execute([$id]);

                // Verifica se a exclusão foi bem-sucedida
                if ($stmt->rowCount()) {
                    // Caminho da pasta do documento
                    $directory = __DIR__ . "/../../../files/documents/{$id}";
                    ...
                    // Retorno de sucesso
                    echo json_encode(['status' => 'success', 'message' => 'Documento e pasta deletados com sucesso.']);
                    $_SESSION['msg'] = [
                        'status' => 'success',
                        'alert' => 'primary',
                        'title' => 'Sucesso',
                        'message' => 'Documento e pasta deletados com sucesso.'
                    ];
                } else {
                    // Rollback em caso de falha
                    $conn->rollBack();
                    throw new Exception("Falha ao deletar o documento no banco de dados.");
                }
            } else {
