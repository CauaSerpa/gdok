    ...
    try {
        ...
        $query = "
            SELECT 
                sc.id, 
                sc.name, 
                sd.name AS depto_name
            FROM tb_sending_categories sc
            LEFT JOIN tb_sending_departments sd ON sd.id = sc.department_id
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $actions = '<a href="' . INCLUDE_PATH_DASHBOARD . 'editar-categoria-envio/' . $row['id'] . '" class="btn btn-sm bg-primary-subtle me-1" data-bs-toggle="tooltip" title="Edit">
