...
if ($_SERVER["REQUEST_METHOD"] === "GET") {
    try {
        ...
        $queryTotal = "SELECT COUNT(*) as total FROM tb_sending_companies WHERE user_id = :user_id";
        ...
        $query = "SELECT id, name, phone, email, responsible, created_at 
                  FROM tb_sending_companies 
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $actions = '<a href="editar-empresa-envio/' . $row['id'] . '" class="btn btn-sm bg-primary-subtle me-1" data-bs-toggle="tooltip" title="Edit">
