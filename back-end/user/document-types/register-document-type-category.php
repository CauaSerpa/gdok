...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['action']) && $_POST['action'] === "register-document-type-category") {
    if (isset($_SESSION['user_id']) && isset($_POST['category_name'])) {
        $user_id = $_SESSION['user_id'];
        $category_name = $_POST['category_name'];

        try {
            $stmt = $conn->prepare("INSERT INTO tb_document_type_categories (user_id, name) VALUES (?, ?)");
