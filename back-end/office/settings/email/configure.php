<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['action']) && $_POST['action'] === "update") {
        if (isset($_SESSION['user_id'])) {
            $id = $_POST['office_id'];
            $channel = 'email';
            $is_active = ($_POST['notify_email'] == 'on') ? true : false;
            $contact = $_POST['office_email'];
            $send_type = $_POST['email_frequency'];
            ...
            try {
                ...
                $stmt = $conn->prepare("SELECT COUNT(*) FROM tb_office_notification_settings WHERE office_id = ? AND channel = 'email'");
                $stmt->execute([$id]);
