<?php
    ...
    if ($stmt->rowCount()) {
        ...
        $stmt = $conn->prepare("
            SELECT c.*, cu.role
            FROM tb_users u
            JOIN tb_company_users cu ON cu.user_id = u.id
            JOIN tb_companies c ON c.id = cu.company_id
            WHERE u.id = ?
            LIMIT 1
        ");
        $stmt->execute([$_SESSION['user_id']]);
...
<div id="app-layout">
    ...
    <!-- Left Sidebar Start -->
    <?php
        include('template-parts/portal-sidebar.php');
    ?>
    <!-- Left Sidebar End -->
    ...
    <div class="content-page">
        <div class="content">
            ...
            <div class="container-fluid">
                ...
                <!-- page content -->
                <?php
                    $url = ($url == "c") ? "category" : $url;

                    // Inclui página
                    if (file_exists('pages/portal-cliente/' . $url . '.php')) {
                        include('pages/portal-cliente/' . $url . '.php');
                    } else {
                        // A página não existe
